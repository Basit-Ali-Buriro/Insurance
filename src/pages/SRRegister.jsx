...
export default function SRRegister({ searchFilter, clearSearchFilter }) {
  const toast = useToast();
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      toast('Failed to load SR data', 'error'); 
    } finally { 
      setLoading(false); 
    }
  }, []);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) { 
        toast(`SR ${modal.mode === 'create' ? 'created' : 'updated'}`, 'success'); 
  ...
  return (
    <div className="space-y-6">
      ...
      <Modal 
        ...
        title={modal.mode === 'create' ? 'Add SR Record' : 'Edit SR Record'} 
        size="lg"
        footer={<>
          <button 
      ...
      >
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="form-group">
              <label className="form-label required">SR Name</label>
              <input 
                className={`bg-surface-deep border border-border-subtle text-on-surface rounded p-2 focus:ring-2 focus:ring-primary focus:outline-none w-full ${errors.sr_name ? 'border-error' : ''}`}
      ...
      <ConfirmDialog 
        ...
        title="Delete SR Record" 
        message="Are you sure you want to delete this Sales Representative? This action cannot be undone." 
      />
      ...
      {previewData && (
        <Modal
          ...
          title={`Documents — ${previewData.sr_name || 'Sales Representative'}`}
          size="lg"
          footer={
            <button
              className="flex items-center gap-1.5 bg-surface-variant hover:bg-surface-container-highest text-on-surface transition-colors px-4 py-2 rounded"
